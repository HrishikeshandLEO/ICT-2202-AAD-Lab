...
public class ASPmain {
    ...
    public static void calculate(ArrayList<ActivitySelectionProblem>list){

        System.out.println(list.get(0).getActivity()+" ");
            int previous=list.get(0).getStart();
            for(int i=1;i<list.size();i++)
            {
                if(list.get(i).getStart()>=previous){
                    System.out.println(list.get(i).getActivity()+" ");
                    previous=list.get(i).getFinish();
                }
            }
    }
